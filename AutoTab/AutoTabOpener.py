...
# turns on and off the console print statements
prntButtonPressed = False
...
class Window(Frame):
    ...
    def init_window(self):
        ...
        entertainTabsButton = Button(
            self, text="Entertainment Mode", command=self.entertain_Button)
        entertainTabsButton.place(
            x=entertainTabsButtonX, y=entertainTabsButtonY)

        # start firefox / chrome working tabs button
        workingTabsButton = Button(
            self, text="Working Mode", command=self.working_Button)
        workingTabsButton.place(x=workingTabsButtonX, y=workingTabsButtonY)

    # browser button command
    def entertain_Button(self):
        if prntButtonPressed is True:
